...
async def test_get_all_sales_retorna_lista(async_client: AsyncClient, mock_sale_service):
    """Obtener todas las ventas retorna paginado 200"""
    from tests.helpers import build_sale_model
    ventas = [
    ...
    mock_sale_service.count_all.return_value = 2
...
async def test_get_sales_con_paginacion(async_client: AsyncClient, mock_sale_service):
    ...
    mock_sale_service.get_all.return_value = ventas
    mock_sale_service.count_all.return_value = 1
...
async def test_get_sales_vacia_retorna_lista_vacia(async_client: AsyncClient, mock_sale_service):
    ...
    mock_sale_service.count_all.return_value = 0
...
@pytest.mark.asyncio
async def test_get_sales_con_filtro_fechas(async_client: AsyncClient, mock_sale_service):
    """Obtener ventas con filtro por fechas retorna paginado 200"""
    from tests.helpers import build_sale_model
    ventas = [build_sale_model(id=1, total=1200.0)]
    mock_sale_service.get_all.return_value = ventas
    mock_sale_service.count_all.return_value = 1

    response = await async_client.get("/ventas/?fecha_desde=2026-01-01&fecha_hasta=2026-01-31")
...
@pytest.mark.asyncio
async def test_get_sales_rango_fechas_invalido_retorna_422(async_client: AsyncClient):
    """fecha_desde > fecha_hasta retorna 422 (validator en router)"""
    response = await async_client.get("/ventas/?fecha_desde=2026-02-01&fecha_hasta=2026-01-01")
    # Note: HTTPException with 422 se convierte en 400 en algunos casos
    # Lo importante es que se rechace la petición
...
@pytest.mark.asyncio
async def test_update_sale_ok_retorna_200(async_client: AsyncClient, mock_sale_service):
    """Actualizar venta existente retorna 200"""
    from tests.helpers import build_sale_model

    venta_actualizada = build_sale_model(id=1, total=2000.0)
    mock_sale_service.update.return_value.error = None
    mock_sale_service.update.return_value.value = venta_actualizada

    payload = {
        "numero_orden": "ORD-UPDATED",
        "items": [{"producto_id": 1, "cantidad": 2, "precio_unitario": 1000.0}],
    }

    response = await async_client.put("/ventas/1", json=payload)
...
@pytest.mark.asyncio
async def test_update_sale_no_existente_retorna_404(async_client: AsyncClient, mock_sale_service):
    """Actualizar venta inexistente retorna 404"""
    mock_sale_service.update.return_value.error = "Venta 999 no encontrada"
    mock_sale_service.update.return_value.status_code = 404
    ...
    response = await async_client.put("/ventas/999", json=payload)
    assert response.status_code == 404


@pytest.mark.asyncio
async def test_update_sale_sin_precio_unitario_retorna_400(async_client: AsyncClient, mock_sale_service):
    """Si falta precio_unitario en un item, el servicio devuelve 400 y el router lo propaga"""
    mock_sale_service.update.return_value.error = "Se debe proporcionar precio_unitario para cada item en la actualización"
    mock_sale_service.update.return_value.status_code = 400
...
@pytest.mark.asyncio
async def test_delete_sale_ok_retorna_204(async_client: AsyncClient, mock_sale_service):
    """Eliminar venta existente retorna 204"""
    mock_sale_service.delete.return_value.error = None
    mock_sale_service.delete.return_value.status_code = 204

    response = await async_client.delete("/ventas/1")
    assert response.status_code == 204


@pytest.mark.asyncio
async def test_delete_sale_no_existente_retorna_404(async_client: AsyncClient, mock_sale_service):
    """Eliminar venta inexistente retorna 404"""
    mock_sale_service.delete.return_value.error = "Venta 999 no encontrada"
    mock_sale_service.delete.return_value.status_code = 404

    response = await async_client.delete("/ventas/999")
    assert response.status_code == 404
